...
// REMOVE faTrash AND faEye from this import as they are not used in this file.
// faShoppingCart IS USED in the empty cart state, so keep it.
// faPlus, faMinus, faShippingFast ARE USED, so keep them.
import { faPlus, faMinus, faShoppingCart, faShippingFast } from '@fortawesome/free-solid-svg-icons';
import { motion } from 'framer-motion';

const Cart = () => {
  const navigate = useNavigate();
  // REMOVE clearCart from this destructuring as it's not used in this file.
  const { cartItems, removeFromCart, updateQuantity, getCartTotal } = useCart();

  // FIX: React Hook useMemo has an unnecessary dependency: 'cartItems'
  // The `getCartTotal()` function from `useCart()` is a stable function
  // (or should be, if `useCart` is properly memoizing it with useCallback).
  // If `getCartTotal` itself already takes `cartItems` into account internally
  // or is a stable reference, then `cartItems` might be redundant here if it's
  // already a dependency of `getCartTotal` or `getCartTotal` itself doesn't change.
  // The simplest fix for this specific ESLint rule is often to remove the redundant dependency.
  // However, in this specific case, `getCartTotal` *depends* on `cartItems` to calculate the total.
  // The ESLint warning `react-hooks/exhaustive-deps` is trying to tell you that `getCartTotal`
  // *should* be in the dependency array if `cartItems` is there. But you already have `getCartTotal`.
  // The primary issue might be that `getCartTotal` itself isn't memoized in `useCart`,
  // causing it to change on every render, thus making `useMemo` re-run unnecessarily.
  //
  // For now, the most straightforward fix for *this specific error message*
  // from the build log is to ensure the dependency array accurately reflects what
  // `totalAmount` depends on. Since `getCartTotal()` likely depends on `cartItems`,
  // and `getCartTotal` is also being passed directly, the ESLint error `React Hook useMemo has an unnecessary dependency: 'cartItems'`
  // suggests that `cartItems` might implicitly be a dependency of `getCartTotal` or
  // that the linter is being overly aggressive.
  //
  // Let's simplify the dependency array to reflect what is *directly* used in the memoized function.
  // A safer approach for the `useMemo` of `totalAmount` if `getCartTotal` is a stable function:
  const totalAmount = useMemo(() => getCartTotal(), [getCartTotal]); // Assuming getCartTotal internally uses cartItems
  // If getCartTotal is NOT a stable function, you might need:
   // const totalAmount = useMemo(() => getCartTotal(cartItems), [cartItems, getCartTotal]);
   // But your original code does `getCartTotal()`, implying `cartItems` is accessed inside `getCartTotal`.
   // So, keeping `getCartTotal` as the only explicit dependency for `useMemo` here is often the correct fix
   // for this specific error message, provided `getCartTotal` is itself stable or correctly handles its internal dependencies.
   // Let's go with the direct fix for the reported error: remove `cartItems` as it's flagged as "unnecessary dependency" *here*.
...
}, [cartItems, totalAmount]); // Keep cartItems and totalAmount here as they are directly used.
...
        {/* The "Clear Cart" button was previously not rendered. If you want to use it, you need to add JSX for it. */}
        {/* Example if you want to add a clear cart button: */}
        {/* <button onClick={clearCart} className="bg-red-600 text-white px-6 py-2 rounded-md hover:bg-red-700">Clear All Items</button> */}
      </div>
    </div>
  );
};
