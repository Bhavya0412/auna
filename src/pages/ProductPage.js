...
import { useParams, useNavigate } from "react-router-dom"; // Keep useNavigate as it's used below
...
const ProductPage = () => {
  const { id } = useParams();
  const navigate = useNavigate(); // This is correctly used in handleContinueShopping
...
  // NEW: Define a function that uses `Maps`
  const handleContinueShopping = useCallback(() => {
    navigate('/'); // Example: navigate back to home page
  }, [navigate]);
...
        <p className="mt-4">The product you're looking for doesn't exist or has been removed.</p>
        <button
          onClick={handleContinueShopping} // Use the new function here
          className="mt-6 bg-oliveGreen text-white px-6 py-2 rounded-md hover:bg-darkolivegreen transition-colors"
        >
          Continue Shopping
        </button>
      </div>
